...
Alternatively, run `scripts/generate-and-pr.sh` to do the above and automate the PR.  It takes the same optional version type argument.  It wraps `generate.sh`, detects the old → new spec version, and (after you confirm) bumps the package version and ensures you're on a `gen-<version>` branch, then commits, pushes, and opens a PR against `main` via `git`/`gh`, printing the PR URL. The [`claude`](https://docs.claude.com/en/docs/claude-code/overview) CLI is used only to draft the PR description from the spec diff. Requires the `gh` and `claude` CLIs to be installed and authenticated.
